...
class QKV(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        qkv = self.qkv(x)        
        qkv = rearrange(qkv, '... (qkv h d) -> ... qkv h d', qkv=3, h=self.num_heads, d=self.head_dim)
...
class JointAttention(nn.Module):
    ...
    def forward(self, all_qkv):
        ...
        out = rearrange(out, '... h n d-> ... n (h d)', h=self.num_heads, d=self.head_dim)
...
class Attention(nn.Module):
    def __init__(
        self, dim: int, num_heads: int = 8, qkv_bias: bool = False, qk_norm: bool = False, 
        attn_drop: float = 0., proj_drop: float = 0., attn_type='torch', 
        norm_layer: nn.Module = nn.LayerNorm, is_causal=False
    ) -> None:
        super().__init__()
        assert dim % num_heads == 0, 'dim should be divisible by num_heads'
        self.num_heads = num_heads
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.attn_type == 'flash':
            ...
            out = torch.nn.functional.scaled_dot_product_attention(
                q_t, k_t, v_t, dropout_p=self.attn_drop.p if self.training else 0., is_causal=self.is_causal
            )
...
class ActionPositionEmb(nn.Module):
    def __init__(self, len_t, hidden_dim, theta=10000):
        ...
        self.register_buffer("position_encoding", self.rotary_position_embedding(len_t, hidden_dim, theta=self.theta))
    def forward(self, x):
...
class VideoRopePosition3DEmb(nn.Module):
    ...
    def generate_embeddings(
        ...
        # else:
        half_emb_t = torch.outer(self.seq[:T] / self.base_fps * self.base_fps, temporal_freqs)
...
class VideoPositionEmb(VideoRopePosition3DEmb):
    def __init__(self,
        ...
        super().__init__(head_dim=head_dim,
            ...
            theta=theta ,
            device=device)
        self.register_buffer('position_encoding', self.generate_embeddings(
            (len_t, len_h, len_w),
...
class AttentionProfiler:
    ...
    def _generate_inputs(self, batch_size: int, seq_len: int, dim: int, dtype=torch.float32) -> torch.Tensor:
        """Generate random input tensors for profiling."""
        return torch.randn(batch_size, seq_len, dim, device=self.device, dtype=dtype)
    
    def _create_attention_module(self, attn_type: str, dim: int, num_heads: int, dtype=torch.float32) -> Attention:
    ...
    def profile_single_config(
        ...
        for attn_type in self.attention_types:
            try:
                ...
                model = self._create_attention_module(attn_type, dim, num_heads).to(dtype)
                ...
                print(f"Config [B={batch_size}, L={seq_len}, D={dim}, H={num_heads}] "
    ...
    def profile_varying_hidden_dim(
        ...
        assert len(dims) == len(num_heads_list), "dims and num_heads_list must have same length"
...
if __name__ == '__main__':
    ...
    print(profiler.profile_varying_sequence_length(16,  [256, 512, 768, 1024, 2048], 512,8, dtype=torch.float32))

    print('vary seq len', 'torch.bfloat16')
    print(profiler.profile_varying_sequence_length(16,  [256, 512, 768, 1024, 2048], 512,8, dtype=torch.bfloat16))

    print('vary hidden dim', 'torch.float32')
    print(profiler.profile_varying_hidden_dim(16,  512,[256, 512, 768, 1024, 2048], [8, 8, 8,8, 8], dtype=torch.float32))

    print('vary hidden dim', 'torch.bfloat16')
    print(profiler.profile_varying_hidden_dim(16,  512,[256, 512, 768, 1024, 2048],[8,8,8,8,8], dtype=torch.bfloat16))

    print('vary batch size', 'torch.float32')
    print(profiler.profile_varying_batch_size([16, 16, 24, 32], 512,512,8, dtype=torch.float32))

    print('vary batch size', 'torch.bfloat16')
    print(profiler.profile_varying_batch_size([16, 16, 24, 32], 512,512,8, dtype=torch.bfloat16))
    ...
    pos_embedder = VideoRopePosition3DEmb(head_dim=64,len_h=32, len_w=32, len_t=16 , base_fps=30).to('cuda')
