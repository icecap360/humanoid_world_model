...
class MMDiTBlockHyperConnections(MMDiTBlock):
    def __init__(self, token_dim, time_dim, num_heads, skip_context_ff=False):
        ...
        init_hyper_conn_fv_attn, expand_stream_fv_attn, reduce_stream_fv_attn = (
            get_init_and_expand_reduce_stream_functions(1)
        )
        init_hyper_conn_pv_attn, expand_stream_pv_attn, reduce_stream_pv_attn = (
            get_init_and_expand_reduce_stream_functions(1)
        )
        init_hyper_conn_fa_attn, expand_stream_fa_attn, reduce_stream_fa_attn = (
            get_init_and_expand_reduce_stream_functions(1)
        )
        init_hyper_conn_pa_attn, expand_stream_pa_attn, reduce_stream_pa_attn = (
            get_init_and_expand_reduce_stream_functions(1)
        )

        # For feed-forward branch on each stream
        init_hyper_conn_fv_ff, expand_stream_fv_ff, reduce_stream_fv_ff = (
            get_init_and_expand_reduce_stream_functions(1)
        )
        init_hyper_conn_pv_ff, expand_stream_pv_ff, reduce_stream_pv_ff = (
            get_init_and_expand_reduce_stream_functions(1)
        )
        init_hyper_conn_fa_ff, expand_stream_fa_ff, reduce_stream_fa_ff = (
            get_init_and_expand_reduce_stream_functions(1)
        )
        init_hyper_conn_pa_ff, expand_stream_pa_ff, reduce_stream_pa_ff = (
            get_init_and_expand_reduce_stream_functions(1)
        )
...
if __name__ == "__main__":
    ...
    timesteps = t_embedder(
        torch.tensor([[5], [6], [7], [8], [9]], dtype=torch.float32)
        .unsqueeze(-1)
        .unsqueeze(-1)
    )
